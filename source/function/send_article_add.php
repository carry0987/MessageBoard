...
if(isset($_POST['submit'])) {
  ...
  if($OK === '1') {
      ...
      function countdown() {
          ...
          if (s.innerHTML == 0) {
              window.location = "'.$base_url.'/board.php?board_id='.$Board_id.'";
          } else {
              setTimeout("countdown()", 1000);
          }
      }
      ...
      echo '<br />';
      echo '<a class="ifnorefresh" href="'.$base_url.'/board.php?board_id='.$Board_id.'">'.$lang_no_refresh.'</a>';
      echo '</div>';
    } else {
...
<script>
  ...
  if($AnonymousErr !== 'wtf')
  {
    echo 'alert("'.$AnonymousErr.'");location.href="'.$base_url.'/login.php";';
  }
  ?>
</script>
