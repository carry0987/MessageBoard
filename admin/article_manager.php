...
if(isset($_GET['page'])) {
  if($_GET['page'] == '1' || $_GET['page'] == '0' || $_GET['page'] == '') {
    header('Location: ./admin_article_manager.php');
  } else {
    $page = $_GET['page'];
  }
} else {
...
echo '<h1 style="text-align: center; margin: 0;">'.$lang_article_manager.'</h1>';

  if($result->num_rows > 0) {
      echo '
    <form action="./function/article_delete.php" method="post">
      <div class="box">
        <table>
        ';
      ...
      echo '
            <tbody>
              <tr>
                ...
                <th>
                  <a href=./article_edit.php?id='.$row["id"].'>'.$row["title"].'</a>
                </th>
                <td class="by">
                  <cite>
                    <a>'.$row["username"].'</a>
